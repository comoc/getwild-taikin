...
├── scripts/speak.sh    TTS発話スクリプト（macOS / say）
...
```bash
# 既定フレーズを発話（日本語ボイス Kyoko/Otoya があれば優先）
bash scripts/speak.sh

# フレーズ指定
bash scripts/speak.sh --phrase "Hey Siri, GET WILDを流して"

# 音を出さず合成だけ確認（AIFF書き出し）
bash scripts/speak.sh --out /tmp/gw_test.aiff
```
...
OS のスケジューラに発話スクリプトを登録すると、Claude もセッションも不要になります。
...
```bash
# cron に平日18:00で登録する例
(crontab -l 2>/dev/null; echo "0 18 * * 1-5 bash /path/to/getwild-taikin/scripts/speak.sh") | crontab -
```

launchd を使う場合は `StartCalendarInterval` 付きの plist を `~/Library/LaunchAgents/` に置く（詳細は SKILL.md 参照）。

## 動作環境

- **Windows**（PowerShell / System.Speech による TTS）または **macOS**（`say` コマンド）
