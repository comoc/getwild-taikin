...
description: >-
  ...
  時刻の監視はこのセッション自身が ScheduleWakeup で行う。Windows(PowerShell)・macOS(say)対応。
---
...
- **macOS**（`say` コマンド）:
  ```
  bash "<スキルのベースディレクトリ>/scripts/speak.sh"
  ```

既定フレーズはどちらも「ヘイSiri、GET WILDを流して」。別フレーズを指定したいときは
`-Phrase "..."`（ps1）／`--phrase "..."`（sh、`-Phrase` も可）を付ける。
どちらのスクリプトも日本語ボイスがあれば優先して発話する
（Windowsは音量を最大化、macOSはシステム音量に従う）。

発話後、「Get Wildを再生しました。今日も一日おつかれさまでした。」と短く伝えて終了する。ここは演出を盛らない。

## 動作確認（音を出さずにテスト）

セットアップ前に合成が通るか確かめたいときは、音声ファイルに書き出す（スピーカーは鳴らない）：

```
# Windows
pwsh -File "<スキルのベースディレクトリ>/scripts/speak.ps1" -OutFile "%TEMP%/gw_test.wav"
# macOS
bash "<スキルのベースディレクトリ>/scripts/speak.sh" --out "/tmp/gw_test.aiff"
```
...
セッション常駐が煩わしく、毎営業日に自動で鳴らしたいだけなら、OS のスケジューラに
発話スクリプトを登録するほうが確実。ユーザーが希望したらこの方式に切り替える。
...
**macOS（launchd）**: 次の plist を `~/Library/LaunchAgents/com.getwild.taikin.plist` に置いて
`launchctl load` する（`StartCalendarInterval` の Hour/Minute を退勤時刻に合わせる）。

```xml
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
  <key>Label</key><string>com.getwild.taikin</string>
  <key>ProgramArguments</key>
  <array>
    <string>/bin/bash</string>
    <string>/path/to/getwild-taikin/scripts/speak.sh</string>
  </array>
...
（`cron` に `0 18 * * 1-5 bash /path/to/scripts/speak.sh` を登録しても良い。）
